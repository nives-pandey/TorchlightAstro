...
export default function DailyGuidance({ guidance, compact = false }: DailyGuidanceProps) {
  ...
  if (compact) {
    return (
      <Card className="sanctuary-card">
        <CardHeader>
          <CardTitle className="text-yellow-600 text-lg">Today's Cosmic Energy</CardTitle>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex justify-between text-xs">
            <div>
              ...
              <span className="text-yellow-600 ml-1">{guidance.optimalTiming.best}</span>
            </div>
            <div>
              <span className="text-gray-400">Lucky:</span>
              <span className="text-yellow-600 ml-1">{guidance.luckyElements.numbers[0]}</span>
            </div>
          </div>
        </CardContent>
      </Card>
  ...
  return (
    <div className="space-y-6">
        ...
        <CardContent>
          <div className="grid gap-4">
            {guidance.transits.map((transit, index) => (
              <div key={index} className="flex items-center justify-between p-3 bg-black/20 rounded-lg border border-yellow-600/10">
                ...
                <Badge 
                  variant="outline" 
                  className={`ml-3 ${
                    transit.timing.includes('peak') ? 'border-yellow-600 text-yellow-600' :
      ...
      <Card className="sanctuary-card">
        <CardHeader>
          <CardTitle className="text-yellow-600">Daily Life Areas</CardTitle>
          <CardDescription className="text-gray-400">
          ...
          <CardContent className="space-y-4">
                ...
                {guidance.luckyElements.colors.map((color, index) => (
                  <div 
                    ...
                    className={`w-6 h-6 rounded-full border border-yellow-600/30 ${
                      ...
                      color === 'gold' || color === 'yellow' ? 'bg-yellow-600' :
                      color === 'green' ? 'bg-green-600' :
                      color === 'red' ? 'bg-red-600' :
                      color === 'purple' ? 'bg-yellow-600' :
                      color === 'white' ? 'bg-purple-100/20 border-yellow-500/30' :
                      'bg-gray-600'
                    }`}
                    title={color}
                  ></div>
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                ...
                <div className="text-yellow-600">{guidance.luckyElements.numbers.join(', ')}</div>
              </div>
              <div>
                <div className="text-white font-medium">Direction</div>
                <div className="text-yellow-600">{guidance.luckyElements.direction}</div>
              </div>
            </div>
            <div>
              <div className="text-white font-medium">Element Focus</div>
              <div className="text-yellow-600">{guidance.luckyElements.element}</div>
            </div>
          </CardContent>
        </Card>
      </div>
